...
func AccountCreateH(w http.ResponseWriter, r *http.Request) {
	...
	user, err = ctx.Storage.User().Insert(user)
	if err != nil {
		ctx.Log.Error(err)
		err.Http(w)
		return
	...
	account, err = ctx.Storage.Account().Insert(account)
	if err != nil {
		ctx.Log.Error(err)
		err.Http(w)
		return
	...
	sw := new(SessionView)
	var errencode error
	sw.Token, errencode = model.NewSession(account.UUID, ``, *rq.Username, *rq.Email).Encode()
	if errencode != nil {
...
func AccountGetH(w http.ResponseWriter, r *http.Request) {
	...
	account, err := ctx.Storage.Account().GetByID(session.Uid)
	if err != nil {
		ctx.Log.Error(err)
		err.Http(w)
		return

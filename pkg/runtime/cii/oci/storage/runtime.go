...
func (r *runtimeService) createContainerOrPodSandbox(systemContext *types.SystemContext, podName, podID, imageName, imageAuthFile, imageID, containerName, containerID, metadataName, uid, namespace string, attempt uint32, idMappings *idtools.IDMappings, labelOptions []string, isPauseImage bool) (ci ContainerInfo, retErr error) {
	...
	if img == nil && errors.Cause(err) == storage.ErrImageUnknown && isPauseImage {
		...
		fmt.Println(fmt.Sprintf("couldn't find image %q, retrieving it", image))
		...
		fmt.Println(fmt.Sprintf("successfully pulled image %q", image))
	}
	...
	if err != nil {
		if metadata.Pod {
			fmt.Println(fmt.Sprintf("failed to create pod sandbox %s(%s): %v", metadata.PodName, metadata.PodID, err))
		} else {
			fmt.Println(fmt.Sprintf("failed to create container %s(%s): %v", metadata.ContainerName, containerID, err))
		}
		return ContainerInfo{}, err
	}
	if metadata.Pod {
		fmt.Println(fmt.Sprintf("created pod sandbox %q", container.ID))
	} else {
		fmt.Println(fmt.Sprintf("created container %q", container.ID))
	}
	...
	defer func() {
		if retErr != nil {
			if err2 := r.storageImageServer.GetStore().DeleteContainer(container.ID); err2 != nil {
				if metadata.Pod {
					fmt.Println(fmt.Sprintf("%v deleting partially-created pod sandbox %q", err2, container.ID))
				} else {
					fmt.Println(fmt.Sprintf("%v deleting partially-created container %q", err2, container.ID))
				}
				return
			}
			fmt.Println(fmt.Sprintf("deleted partially-created container %q", container.ID))
		}
	}()
	...
	if metadata.Pod {
		fmt.Println(fmt.Sprintf("pod sandbox %q has work directory %q", container.ID, containerDir))
	} else {
		fmt.Println(fmt.Sprintf("container %q has work directory %q", container.ID, containerDir))
	}
	...
	if metadata.Pod {
		fmt.Println(fmt.Sprintf("pod sandbox %q has run directory %q", container.ID, containerRunDir))
	} else {
		fmt.Println(fmt.Sprintf("container %q has run directory %q", container.ID, containerRunDir))
	}
...
func (r *runtimeService) RemovePodSandbox(idOrName string) error {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("failed to delete pod sandbox %q: %v", container.ID, err))
		return err
	}
	return nil
}
...
func (r *runtimeService) DeleteContainer(idOrName string) error {
	...
	err = r.storageImageServer.GetStore().DeleteContainer(container.ID)
	if err != nil {
		fmt.Println(fmt.Sprintf("failed to delete container %q: %v", container.ID, err))
		return err
	}
	return nil
}
...
func (r *runtimeService) SetContainerMetadata(idOrName string, metadata *RuntimeContainerMetadata) error {
	mdata, err := json.Marshal(&metadata)
	if err != nil {
		fmt.Println(fmt.Sprintf("failed to encode metadata for %q: %v", idOrName, err))
		return err
	}
...
func (r *runtimeService) StartContainer(idOrName string) (string, error) {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("failed to mount container %q: %v", container.ID, err))
		return "", err
	}
	fmt.Println(fmt.Sprintf("mounted container %q at %q", container.ID, mountPoint))
...
func (r *runtimeService) StopContainer(idOrName string) error {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("failed to unmount container %q: %v", container.ID, err))
		return err
	}
	fmt.Println(fmt.Sprintf("unmounted container %q", container.ID))
	return nil
}

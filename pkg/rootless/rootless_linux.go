	...
	isRootlessOnce.Do(func() {
		...
		if rootlessUIDInit != 0 {
			...
			if err := os.Setenv("_CONTAINERS_USERNS_CONFIGURED", "done"); err != nil {
				fmt.Println(fmt.Sprintf("failed to set environment variable %s as %s", "_CONTAINERS_USERNS_CONFIGURED", "done"))
			}
			if err := os.Setenv("_CONTAINERS_ROOTLESS_UID", fmt.Sprintf("%d", rootlessUIDInit)); err != nil {
				fmt.Println(fmt.Sprintf("failed to set environment variable %s as %d", "_CONTAINERS_ROOTLESS_UID", rootlessUIDInit))
			}
			if err := os.Setenv("_CONTAINERS_ROOTLESS_GID", fmt.Sprintf("%d", rootlessGIDInit)); err != nil {
				fmt.Println(fmt.Sprintf("failed to set environment variable %s as %d", "_CONTAINERS_ROOTLESS_GID", rootlessGIDInit))
			}
		}
...
func tryMappingTool(tool string, pid int, hostID int, mappings []idtools.IDMap) error {
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		fmt.Println(fmt.Sprintf("error from %s: %s", tool, output))
...
func GetConfiguredMappings() ([]idtools.IDMap, []idtools.IDMap, error) {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("cannot find mappings for user %s: %v", username, err))
	} else {
		uids = mappings.UIDs()
		gids = mappings.GIDs()
	}
...
func becomeRootInUserNS(pausePid, fileToRead string, fileOutput *os.File) (bool, int, error) {
	...
	defer func() {
		if _, err := w.Write([]byte("0")); err != nil {
			fmt.Println(fmt.Sprintf("failed to write byte 0: %q", err))
		}
	}()
	...
	if !uidsMapped {
		fmt.Println(fmt.Sprintf("using rootless single mapping into the namespace. This might break some images. Check /etc/subuid and /etc/subgid for adding subids"))
		setgroups := fmt.Sprintf("/proc/%d/setgroups", pid)
		err = ioutil.WriteFile(setgroups, []byte("deny\n"), 0666)
		if err != nil {
			return false, -1, errors.Wrapf(err, "cannot write setgroups file")
		}
		fmt.Println(fmt.Sprintf("write setgroups file exited with 0"))
		...
		fmt.Println(fmt.Sprintf("write uid_map exited with 0"))
	}
	...
	go func() {
		for s := range c {
			...
			if err := unix.Kill(int(pidC), s.(unix.Signal)); err != nil {
				fmt.Println(fmt.Sprintf("failed to kill %d", int(pidC)))
			}
		}
	}()
...
func SyncQuiet(f *os.File) {
	if err := f.Sync(); err != nil {
		fmt.Println(fmt.Sprintf("unable to sync file %s: %q", f.Name(), err))
	}
}
...
func CloseQuiet(f *os.File) {
	if err := f.Close(); err != nil {
		fmt.Println(fmt.Sprintf("unable to close file %s: %q", f.Name(), err))
	}
}

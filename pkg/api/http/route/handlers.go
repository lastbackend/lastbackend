...
func RouteUpdateH(w http.ResponseWriter, r *http.Request) {
	...
	log.V(logLevel).Debugf("%s:create update route `%s`", logPrefix, nid)
	...
	if err != nil {
		log.V(logLevel).Errorf("%s:create check route exists by selflink `%s` err: %s", logPrefix, ns.Meta.SelfLink, err.Error())
		errors.HTTP.InternalServerError(w)
		return
	}
	if rs == nil {
		log.V(logLevel).Warnf("%s:create route `%s` not found", logPrefix, rid)
		errors.New("route").NotFound().Http(w)
		return
	}
	...
	if err != nil {
		log.V(logLevel).Errorf("%s:create update route `%s` err: %s", logPrefix, ns.Meta.Name, err.Error())
		errors.HTTP.InternalServerError(w)
	}
